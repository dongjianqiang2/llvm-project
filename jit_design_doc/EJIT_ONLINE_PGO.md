...
**与 wrapper inline cache 的兼容性**:在线 PGO 可以同时启用
`-mllvm -ejit-inline-cache=true`。PGO 开启时，Tier-1 不填 wrapper inline
cache，调用继续经过 shared-taskpool 累计 `hitCount`；Tier-2 发布后，当前业务
核首次解析到经过本核执行权限准备的 Tier-2 指针时才填本核 cell，后续调用恢复
无额外 guard 的 wrapper 直达路径。per-core L0 同样在 PGO 开启时停填并通过
`dispatchEpoch` 退役。该协议面向 `ejit_init_pgo()` 的初始化期启用；已缓存
Tier-1 后再动态开启 PGO 仍缺少跨核清空所有私有 wrapper cell 的协议，不受支持。
...
**⚠ v0.11 finding(v0.12 已修)**:原 ORC `lookup(cacheKey, "__profc_"+pgoName)` **失败**--JITDylib 符号 claim 在 `addIRModule` 时据原模块算,而 `__profc_*` 是 transform 内 Gen 生成(addIRModule 后,不在原模块)-> 未被 claim -> lookup 不到(`captureCounterGlobals` 强制 External 也无效,因 claim 已定)。修复方向:不经 ORC lookup,改经码池内存管理器(post-compile 符号地址表)取计数器地址 + `JITDylib::define(absoluteSymbols)` 注册,或 transform 后手动注册。**v0.12 已修**:IRTransformLayer transform lambda 里 runPipeline 后对 `__profc_*/__profd_*` 调 `R.defineMaterializing(...)` 扩展 MR claim -> ORC lookup 可用。集成测试 `OrcLookupAndRealAddrProfileMerge` 验证(真 ORC lookup + 真 addr 读取 + 合成)。Tier-1 捕获路径生产可用。
