...
> **v0.15 变更**:共享 taskpool 增加逐函数 PGO admission。任一时刻最多一个 `funcIndex` 可进入 Tier-1 插桩；其他未获准函数不排编译任务，继续执行 AOT fallback。当前函数按 Tier-1 hit threshold 输出 0/25/50/75/100% 进度，Tier-2 成功发布后释放名额，下一个实际发生 miss 的函数再开始。Tier-2 暂时失败时保留当前名额和 Tier-1，避免旧插桩仍运行时又启动第二个插桩函数。共享 ABI 升至 v9。
