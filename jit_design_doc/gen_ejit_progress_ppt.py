...
BASE_REF_CANDIDATES = ("origin/main", "origin/release/21.x", "release/21.x")
...
def page2():
    ...
    data = [
        ...
        ("⑬", "JIT 优化流水线 + 在线PGO ★", "运行时", "✓", "参数替换→InstCombine→PASS6→L1/L2/L3; 在线PGO(Tier1插桩→Tier2热点重编译→MBP块布局+内联); Inline按PGO决策"),
...
def page3_roadmap():
    ...
    phases = [
        ("近期：验证闭环", "2~4 周", GREEN, [
            ("在线PGO 收尾", "共享PGO触发已通; 真ejit_init端到端集成; +640KB体积决策"),
