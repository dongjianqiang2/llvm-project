...
/* ---------- period×dim fold：可增量追加；operand 可缺省（→ 该 dim 的 spec）；示例中随所属 period 归组声明（§3.5，O-6）；v0.4.18 由 DEFINE_FOLD 更名（宏名显式携带 (period, dim) 绑定） ---------- */
#define DEFINE_PERIOD_DIM_FOLD(...) EJIT_FOLD_SELECT(EJIT_NARG(__VA_ARGS__))(__VA_ARGS__)
#define EJIT_FOLD_SELECT(n) EJIT_CAT(DEFINE_PERIOD_DIM_FOLD_, n)
#define DEFINE_PERIOD_DIM_FOLD_3(period_name, dim_name, fold_op) \
  __attribute__((ejit_period_fold(#period_name, #dim_name, fold_op))) \
  static int EJIT_CAT(ejit_fold_decl_anchor_, __COUNTER__) \
      __attribute__((unused));
#define DEFINE_PERIOD_DIM_FOLD_4(period_name, dim_name, fold_op, operand) \
...
| `ejit_dim_type` | `StringArgument dim, StringArgument type` | 每 dim 恰好一次；type ∈ 白名单 `{half-static, dynamic}`（预留 `static`）；**语义（D14）**：dynamic = 降级候选——wrapper 降级链轮2 去掉该 dim（不特化、数据查表），轮1 仍正常参与全特化；轮2 半特化配独立 icache 层（Tier-1，v0.4.19）（§6.3） |
...
1. **fold 整行缺省**：period 的某 dim 无 `DEFINE_PERIOD_DIM_FOLD` 行 → 变换为 identity。
2. **operand 缺省**：`DEFINE_PERIOD_DIM_FOLD(p, d, EJIT_FOLD_MOD)` → operand 取该 dim 的 spec。
...
DEFINE_PERIOD_DIM_FOLD(period23, dim3, EJIT_FOLD_MOD, 10)   /* fold 随所属 period 归组声明（O-6） */
...
**轮2 快路径（Tier-1 独立 icache，v0.4.19 定案）**：降级链的每一轮都配置自己的 icache 层——轮2 的 key 空间是 **static 子空间**：per-static-dim 按轮2 的 period 集合重算 `impl_spec_d2 = min(spec, LCM2)`（LCM2 = 该 dim 在 static 相关 periods 上的 fold 的 LCM，去掉 dynamic 相关 periods 后重新推导），轮2 格子数组维度 = static dims、尺寸 = `Π 2^⌈log2 impl_spec_d2⌉`、下标 = 轮2 specId（`dim_v % impl_spec_d2`，wrapper AOT 内嵌移位常数；与轮1 分量模数相等时 AOT 常量判断直接复用，零额外指令）。格子形态与轮1 完全一致（v0.4.14 间接引用：存 bucket slot 地址，同一 bucket/slot/双位协议/tick 节流/referenced 保护）。wrapper 调用链 = 轮1 快路径 → 轮1 慢路径 → 轮2 快路径 → 轮2 慢路径 → AOT 兜底——**轮1 命中路径零新增开销**（轮2 段仅在降级后执行）。动态数据高频变化时轮1 反复失效、轮2 成为稳态快路径——这是轮2 icache 的价值。仅存在 dynamic dim 时生成轮2 层（全 static 函数无降级链，轮2 = 轮1）。
...
| 组件 | 改动 |
|------|------|
| EJitWrapperGen.cpp | 多段 compile_or_get 调用 + 失败分支；轮2 的 specs/periods 集合静态推导（去 dynamic dim 及其相关 period）；**轮2 快路径段（独立 urem 链 + 轮2 格子访问，模数相等复用轮1 分量，v0.4.19）**；AOT 兜底调用 |
...
读者（wrapper 快路径，每次调用）:
    ...
    if (p) call p
写者 = cachePublish 内嵌流程（**唯一发布路径，v0.4.20 合流定案**——无独立发布流程）:
    bucketWrite(B)                       // 写锁：慢路径读者排空（保留，职责 = 读完整性）
    提交门（锁内）：版本快照复核 + generation gate → 失败 VersionMismatch 返回
    找 target（同 identity 覆写 / firstEmpty / evict）→ 无 → Failed
    释放超门限副位（tick 差 > 门限，见 tick 管理段，收集 oldPtrs）  // 腾空位
    无空位 → 丢弃编译结果（A4 兜底）
    ptr[空位].store(新指针, relaxed)     // 先写指针
    tag.store(空位, release)            // 后置主位（发布）
    bucketWriteRelease(B) → 锁外 releaseFn_(oldPtrs)   // 旧指针延迟释放
    // 格子不需要动——slot 地址不变；icacheFill（写格子 + 置 referenced）为同一流程后续步骤
```
...
**与版本失效合流（A3 定案，v0.4.14 修订为 O(1) 失效）**：版本失效不再只靠慢路径复核——**deactivate(period, cellIdx) = 版本表失效 + 刷新该实例的 slot 置 `tag = INVALID` + bump L0 epoch 粗失效**。**格子无需遍历**：快路径格子全部指向 slot（间接引用模型），slot 一处失效，所有引用它的格子自动视为空。**slot 定位（v0.4.20）**：slot 按（函数, key）存放、同一 period 实例可被多个 entry 函数引用（每函数各有轮1 slot，static period 还有轮2 slot）——deactivate 经 PeriodRegistry 的 **period → 引用函数列表**（声明期收集）定位：遍历列表、每函数按 key hash **O(1) 直达**刷 1-2 个 slot，**不扫描格子数组、不扫描桶内槽位**——"O(1) 失效"的准确表述 = **O(引用函数数)，每函数 O(1) 直达**（周期切换低频可接受）。此后快路径读 INVALID → 视为空 → 落慢路径复核 → 版本不匹配 → 触发编译 → 新 fnPtr 发布（置主位，格子仍无需动）→ 快路径自动恢复，**activate 无需发布任何东西**（版本表 bump 即驱动重编译）。竞态无害性：deactivate 写入 tag 的微秒级窗口内旧代码仍执行——但此刻数据尚未写入，旧代码配旧数据**一致无害**；数据写入后任何路径（快/慢）都拿不到旧指针。INVALID 且副位未超门限的延迟编译期间，调用走降级链（轮2 查表永远新鲜）——正确性不受影响。与**指针失效**（替换/销毁，标签翻转）两个维度，组合使用。**轮2 双槽位（v0.4.19）**：存在降级链时，deactivate(static period) = 版本表失效 + 刷轮1 该实例 slot + **刷轮2 对应 slot**（由 cellIdx 解码 static 分量 → 轮2 specId2 推得，O(1) 常数次操作，数学性质 §6.2 在轮2 子空间成立）；deactivate(dynamic 相关 period) 只刷轮1——轮2 slot 快照不含 dynamic period 版本行，版本不牵连（§6.3）。**写者互斥升级（v0.4.20）**：slot 写入者 = cachePublish（发布）与 deactivate（失效，刷 tag=INVALID）**两个**——走**同一 bucketWrite 写锁串行**：deactivate 在前 → publish 提交门复核（版本已失效）失败不发布；publish 在前 → deactivate 的 INVALID 后写覆盖主位——**无需二次复核**。deactivate 低频（周期切换），拿写锁可接受。deactivate **不释放任何指针**（tag=INVALID 后两槽位均成副位，指针驻留由 tick 懒释放管理）。
...
**前提声明（显式）**：① **写者互斥**——slot 写入者 = cachePublish（发布）与 deactivate（失效刷 INVALID），同一 bucketWrite 写锁串行（v0.4.20；副位仅编译线程访问，天然无并发）；② **在途调用时长上界**——读者从 load 到 call 返回的存活期 < tick 门限（嵌入式 WCET 分析可得），保证释放副位时无在途读者持有该指针。
...
**命题 P1 —— type 的应用语义（D14/§6.3 已细化；v0.4.19 立项，转本次方案范围）**：

- **应用模型（已定案）**：type 标在 dim 上；wrapper 两级降级链（轮1 全特化 → 轮2 去 dynamic 半特化 → AOT 兜底）；dim 处理三场景（参数替换 / 折叠表达式替换 / 全不替换），替换集合 per 请求。
- **轮2 独立 icache（v0.4.19 定案）**：轮2 快路径配 Tier-1 层——static 子空间 key（`impl_spec_d2 = min(spec, LCM2)`）+ 与轮1 相同的格子/双位协议/失效机制（§6.3）。
...
- 余项暂缓：是否引入第三种 `static` type（编译期恒定，§3.3 白名单已预留）。
...
| ~~O-12 候选~~（维度级生命周期） | ~~"period 下各 dim 独立激活"~~ **已决（v0.4.19）：不成立，关闭**——period 是整体：生命周期粒度 = period 实例（cellIdx），dim 只是特化维度、无使能/版本概念（D12）；lc 缺组成 dim → error（§3.3）已封闭部分激活路径，"各 dim 独立激活"无独立语义（dim 隶属多个 period 时无法定义） |
...
| D8 | 分层回退已定案并立项（D14/§6.3：type 标 dim + wrapper 两级降级链 + **轮2 独立 icache（v0.4.19）**，P1 转本次方案范围） |
...
按组件分三类，共 30 项。状态列：**待办** = 未开工；**本次方案范围** = v0.4.x 复合 period 方案的直接改动；**未来命题** = 与 P1/P2/O 编号关联的延后项；**代码侧** = 独立于本方案的小修正。
...
| 2.4 | PeriodRegistry 生成 | 声明 → 注册表行：periodId、声明实例空间（Π impl_spec_p）、cellIdx 映射；**period → 引用函数列表（deactivate 定位索引，v0.4.20）** | 待办（本次方案范围） |
...
| 2.10 | wrapper 降级链生成（轮2 + AOT 兜底） | 存在 dynamic dim 时生成：轮2 specs/periods 集合静态推导（去 dynamic dim 及其相关 periods）、per-static-dim 重算 impl_spec_d2 = min(spec, LCM2)、轮2 快路径段（独立 urem 链 + 轮2 格子访问，模数相等时复用轮1 分量）、轮2 慢路径调用、AOT 兜底调用；轮1 命中路径零新增开销（D14/§6.3，v0.4.19 Tier-1） | 待办（本次方案范围） |
...
| 3.10 | 双位协议落地 | slot 双位 ptr[2] + tag 三态（0/1/INVALID）；快路径 = 格子 load（slot 地址）→ tag.load(acquire) → INVALID 走慢路径 → ptr[t].load(relaxed) → call；写入流程 = **cachePublish 内嵌（提交门后锁内：释放超门限副位 → 空位写新指针 → 置主位；锁外 releaseFn_(oldPtrs)）——唯一发布路径**（格子不动；icacheFill 为同流程后续）（§6.4，v0.4.20） | 待办（本次方案范围） |
| 3.11 | deactivate 刷 slot | deactivate(period, cellIdx) = 版本表失效 + **经 period → 引用函数列表定位（每函数按 key hash O(1) 直达，不扫描格子/槽位）**刷轮1/轮2 slot 置 tag=INVALID + bump L0 epoch——O(引用函数数)（v0.4.14/20）；与 cachePublish 同一 bucketWrite 写锁串行；不释放指针 | 待办（本次方案范围） |
...
| 3.16 | 轮2 icache 分配与填充 | 存在 dynamic dim 时按 Π 2^⌈log2 impl_spec_d2⌉ 分配轮2 格子数组（独立于轮1，D13 各级独立预算）；轮2 慢路径编译成功后 icacheFill(轮2 key) 写 slot 地址并置 referenced（§6.3，v0.4.19） | 待办（本次方案范围） |
| 3.17 | deactivate 双槽位失效 | static period deactivate = 版本表失效 + 刷轮1 该实例 slot + 刷轮2 对应 slot（cellIdx 解码 static 分量 → specId2 推得，O(1)）；dynamic 相关 period 只刷轮1（轮2 版本不牵连）；re-init 清空本核轮1+轮2 格子（§6.4，v0.4.19） | 待办（本次方案范围） |

**命题与开放问题状态**：

- **P1**（type 应用语义 / 分层回退）：**本次方案范围（v0.4.19 立项）**——降级链（D14/§6.3）+ 轮2 独立 icache（Tier-1，§6.3/3.16/3.17/2.10）；`static` type 仍预留（§3.3）。
...
- **O-12**：已决（v0.4.19）——维度级生命周期**不成立，关闭**：period 是整体，生命周期粒度 = period 实例（cellIdx），dim 无使能/版本概念（D12）。
...
| type | dim 的稳定性分类（half-static / dynamic，预留 static）——应用语义见 §6.3（D14 降级链 + v0.4.19 轮2 icache） |
