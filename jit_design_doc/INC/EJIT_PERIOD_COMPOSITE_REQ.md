...
按组件分三类，共 27 项。状态列：**待办** = 未开工；**本次方案范围** = v0.4.x 复合 period 方案的直接改动；**未来命题** = 与 P1/P2/O 编号关联的延后项；**代码侧** = 独立于本方案的小修正。
