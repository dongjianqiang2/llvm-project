...
public:
  ...
  // XBBR: indices of SHT_LLVM_XBBR_ATTR sections seen in this object.
  // The sections themselves are discarded by the linker (SHF_EXCLUDE —
  // they must not enter the loadable image), but Stage 0 reads them
  // through the raw Elf_Shdr table before that drop.
