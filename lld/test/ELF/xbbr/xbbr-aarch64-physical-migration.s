...
# Phase 2: XBBR physically reorders per-BB InputSections at link time. Three
# functions a (hot, entry_count 1000), c (cold, 1), b (hot, 900), with a hot
# a→b call edge. Input/module order is a, c, b. XBBR clusters the hot pair
# {a,b} (CGProfile a→b edge from !prof) and pushes cold c last, so the linked
# .text order becomes a, b, c — b physically moves before c. This proves the
# sectionOrder→sortISDBySectionOrder mechanism moves BBs, deterministically.
