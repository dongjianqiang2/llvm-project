...
```
┌──────────┐    ┌──────────────┐    ┌──────────────┐    ┌──────────────┐
│  clang   │───▶│ opt-info 收集 │───▶│ MCP REST API │───▶│  源码 patch   │
│  编译    │    │ (JSON 诊断包) │    │  大模型分析   │    │  生成+review  │
└──────────┘    └──────────────┘    └──────────────┘    └──────────────┘
      ▲                                                         │
      │          ┌──────────────┐                               │
      └──────────│  重编译+验证   │◀──────────────────────────────┘
...
| 形态            | 说明                                                                  | 适用场景        |
| ------------- | ------------------------------------------------------------------- | ----------- |
| **Driver 脚本** | Shell/Python 编排：编译→提取 opt-info→调 MCP→patch→重编译，工具链零侵入               | 快速试用、CI 集成  |
| **LLVM Pass** | 新增 `AIMVFeedback` Pass，集成到 opt/llc 管线中，pass 执行时收集诊断并触发 REST 调用      | 深度集成、批量编译   |
| **独立后处理工具**   | 扩展 llvm-opt-report 或独立 Python/Go 工具，解析 YAML/JSON opt-records 离线批量分析 | 离线分析、团队共享报告 |
