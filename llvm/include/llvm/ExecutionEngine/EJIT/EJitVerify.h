...
// runtime helper, libcall entry and the ejit_verify_* C API all disappear,
// ejit_init_verify() among them. A caller that asks for verify mode against a
// runtime built without the flag therefore fails to link, rather than getting
// an ordinary substituting run whose counters all read zero — indistinguishable
// from "nothing diverged", the one answer this mode must never give by accident.
//
//===----------------------------------------------------------------------===//
