...
PreservedAnalyses EJitWrapperGenPass::run(Module &M,
  ...
  M.getOrInsertFunction(
      ...
      FunctionType::get(StatusTy, {I32Ty, PtrTy, I32Ty, PtrTy, PtrTy}, false));
  M.getOrInsertFunction(
  ...
  for (Function *F : EntryFuncs) {
    ...
    // jit_entry: allocas plus the funcIndex guard. Load the registration-
    // backfilled dense funcIndex; while it is still kEJitInvalidFuncIndex
    // (function unregistered / funcIndex capacity exhausted) we must NOT enter
    // the taskpool — branch straight to the AOT fallback.
    IRBuilder<> Builder(JitEntry);
    auto *DimPairTy = StructType::get(I32Ty, I32Ty);
    Value *DimsAlloca = Builder.CreateAlloca(ArrayType::get(DimPairTy, 4),
                                             nullptr, "ejit_dims");
    Value *OutFnAlloca = Builder.CreateAlloca(PtrTy, nullptr, "ejit_out_fn");
    ...
    // jit_call: build the dim pair array [{dimType, instanceId}] * up to 4 and
    // issue the taskpool request with the loaded funcIndex.
    ...
    for (unsigned I = 0; I < DimCount; ++I) {
      ...
      // Load the runtime-assigned dimType slot from this lifecycle's global
      // (filled at registration by the process-global EJitLifecycleRegistry).
      // NOT a baked per-module constant — that is what made the same sorted
      // position alias different lifecycles across modules.
      ...
      if (ArgVal->getType()->isIntegerTy()) {
        ...
        InstanceId = ConstantInt::get(I32Ty, 0);
      }
    ...
    // The dense funcIndex comes from the per-function global the registration
    // backfilled (loaded above) — never a baked constant — so the wrapper and
    // the loader observe the SAME explicit index.
    FunctionCallee CompileFn = M.getFunction(FN_TASKPOOL_COMPILE_OR_GET);
    Value *Status = Builder.CreateCall(
        CompileFn, {FuncIdx, DimsPtr, ConstantInt::get(I32Ty, DimCount),
    ...
    Value *HitStatus = Builder.CreateICmpEQ(Status, ConstantInt::get(I32Ty, 0));
    Value *HasFn = Builder.CreateIsNotNull(OutFn);
    Value *CanDispatch = Builder.CreateAnd(HitStatus, HasFn);
    Builder.CreateCondBr(CanDispatch, JitDispatch, JitFallback);
