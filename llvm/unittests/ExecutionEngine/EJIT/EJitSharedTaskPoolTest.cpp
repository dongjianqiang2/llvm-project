...
TEST_F(SharedTaskPoolTest, PgoBoundPointerIsReadOnlyDuringCompile) {
  ...
  // Tier-1 is executable immediately from the far pool on current main.
...
TEST_F(SharedTaskPoolTest, ColdCompanionRejectsOverlapAndPartialDescriptor) {
  ...
  EXPECT_TRUE(fourK.seals.empty());
  slot->cold = {};
  slot->cold.codeSize = 64; // Nonempty but missing the code/pool addresses.
...
TEST_F(SharedTaskPoolTest, FourKAbiVersionAndRangeFieldSemantics) {
  ...
  // spec5 v18 plus the cold companion; not the spec4 per-cell layout.
  EXPECT_EQ(kEJitSharedAbiVersion, 26u);
